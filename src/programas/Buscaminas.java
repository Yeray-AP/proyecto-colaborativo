...
public class Buscaminas {
    ...
    private static int[][] matriz_tablero; // Uso una matriz secundaria en donde dibujar las banderas, la cual se
...
=======
    private static int fila = -2;
>>>>>>> afee0ef99e975c259100e4238125bc5fa742d8ad
    ...
    public static void juego() {
        ...
        BombasAleatorias();
        Tablero();
        contador_casillas_correctas = 0; // reinicio el contador, para el caso que se quiera iniciar una nueva partida,
                                         // una vez jugada ya una
        bomba_explotada = false; // reinicio la bomba explotada, para el caso que se quiera iniciar una nueva
                                 // partida, una vez jugada ya una
        ...
        while (!victoria && !bomba_explotada) { // Compruebo si ha ganado o ha explotado una bomba
            ...
            victoria = (matriz.length * matriz.length - bombastotales == contador_casillas_correctas); // actualizo el
                                                                                                       // valor de
                                                                                                       // victoria
            if (bomba_explotada) {
    ...
    private static void PonerBandera() {
        ...
        while (fila < 0 || fila > matriz_tablero.length || columna < 0 || columna > matriz_tablero.length) { // Comprobar
            ...
            if (fila < 0 || fila > matriz_tablero.length || columna < 0 || columna > matriz_tablero.length) {
                ...
                System.out.println("");
            } else if (matriz_tablero[fila][columna] == 1) { // No permito poner una bandera encima de una casilla
                                                             // desvelada
    ...
    private static void DesvelarCasillas() {
        fila = -1;
        columna = -1; // Reinicio el valor de las variables
        while (fila < 0 || fila >= matriz.length || columna < 0 || columna >= matriz.length) { // Comprobar que la fila
                                                                                               // y columna son la
                                                                                               // validas en la matriz,
                                                                                               // y asi no peta
    ...
    private static void BombasAleatorias() {
        ...
        for (int bombas = bombastotales; bombas > 0; bombas--) {
            double random_1 = (matriz.length * (Math.random())); // Creo los numeros aleatorios segun la logitud de la
                                                                 // matriz
            int fila_aleatoria = (int) random_1;
            double random_2 = (matriz.length * (Math.random())); // Creo los numeros aleatorios segun la logitud de la
                                                                 // matriz
            int columna_aleatoria = (int) random_2;
            if (matriz[fila_aleatoria][columna_aleatoria] == 0) { // Para comprobar que en esa posicion esta vacia y no
                                                                  // hay una bomba ya
                matriz[fila_aleatoria][columna_aleatoria] = 5;
            } else {
                bombas++; // Se suma uno a bombas ya que se ha descontado una y no se ha puesto la bomba
                          // realmente
            }
        }
    ...
    private static void Tablero() {
        System.out.print("\t");
        for (int i = 0; i < matriz_tablero.length; i++) { // Para mostrar los numeros de las columnas arriba del tablero
                                                          // para que sea más visual
            System.out.print(i + " "); // En la dificultad dificil al haber numeros de dos cifras se descuadra un poco
                                       // pero aun asi creo que suma mucho por ello lo dejo
        }
        System.out.println("\n");
        ...
        for (int filas = 0; filas < matriz_tablero.length; filas++) {
            ...
            for (int columnas = 0; columnas < matriz_tablero[filas].length; columnas++) {
                switch (matriz_tablero[filas][columnas]) {
                    case 1 ->
                        RadardeBombas(matriz, filas, columnas); // La casilla que ha sido marcada con 1 se comprueba las
                                                                // bombas del arlededor y se imprime
                    case 2 ->
                        System.out.print("? ");
                    case 3 ->
                        System.out.print("* ");
                    default -> // En caso de no haber ninguna información sobre esa posición se pone # de
    ...
    private static void RadardeBombas(int matriz[][], int fila, int columna) {
        ...
        for (int i = fila - 1; i <= fila + 1; i++) { // Con este doble for comprueba 9 posiciones, empezando una
                                                     // posicion superior izquierda resepcto a la posición
            for (int j = columna - 1; j <= columna + 1; j++) { // Y termina en una posicion inferior derecha respecto a
                                                               // la posicion, tambien mira la posicion pero si aqui
                                                               // hubiese bomba ya deberia de haber terminado el
                                                               // minijuego
                if (i >= 0 && i < matriz.length && j >= 0 && j < matriz.length) { // Para los casos que la posición sea
                                                                                  // un borde que si no peta por tamaño
                                                                                  // de matriz
                    if (matriz[i][j] == 5) {
                        contador_bombas++;
                    }
        ...
        for (int filas = 0; filas < matriz_tablero.length; filas++) {
            for (int columnas = 0; columnas < matriz_tablero[filas].length; columnas++) {
                ...
                if (matriz[filas][columnas] == 5) {
                    matriz_tablero[filas][columnas] = 3; // Las convierto las minas en numero 3 para que se muestren en
                                                         // el tablero como *
                }
            }
        }
