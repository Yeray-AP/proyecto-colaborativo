...
public class Hundirlaflota {
     ...
     * 6 = Submarino 1
     * 7 = Submarino 2
     * 8 = Lanchas (Al solo tener una posición puedo usar el mismo codigo para
     * todas)
     */
    ...
    private static int portaaviones_jugador[][] = new int[4][3]; // Matrices para almacenar datos de posicion y de
                                                                 // hundimiento
    ...
    public static void juego() {
        ...
        while (true) {
            // Disparo del jugador
            contador_barcos_hundidos_de_la_cpu = Disparo(true, matriz_cpu, portaaviones_cpu, submarino_1_cpu,
                    submarino_2_cpu, contador_barcos_hundidos_de_la_cpu);

            // Disparo de la cpu
            contador_barcos_hundidos_del_jugador = Disparo(false, matriz, portaaviones_jugador, submarino_1_jugador,
                    submarino_2_jugador, contador_barcos_hundidos_del_jugador);
    ...
    private static void ColocarCpu(int tipo) {
        int distancia = 0;
        if (tipo == 5) {
            distancia = 3; // Pongo 3 para el portaaviones porque es la distancia que tiene que haber de la
                           // casilla inicial a la fina, aunque en realidad tiene 4 casillas
        } else if (tipo == 6 || tipo == 7) {
            distancia = 2; // Pongo 2 para el submarino porque es la distancia que tiene que haber de la
                           // casilla inicial a la fina, aunque en realidad tiene 3 casillas
        }
        ...
        while (!colocar) {
            ...
            if (tipo == 8) {
                matriz_cpu[fila_aleatoria][columna_aleatoria] = 8; // Si es una lancha se coloca asi y se rompe el bucle
                break;
            }
            ...
            if (horizontal_vertical == 0) { // Si es 0 el barco se colocar en vertical, es decir las filas son iguales
                ...
                if (Comprobar_Espacio_Barco(fila_inicial, columna_inicial, fila_final, columna_final, tipo)) {
    ...
    private static boolean Portaaviones() {
        ...
        while (!colocar) { // pedir las posicion de inicio y final del portaviones
            ...
            if (Comprobar_Espacio_Barco(fila_inicial, columna_inicial, fila_final, columna_final, 5)) { // el
                                                                                                        // portaviones
                                                                                                        // encaja
                ...
                if (colocar == false) {
                    System.err.println(
                            "No hay espacio disponible en las posiciones marcadas porque hay barcos ya, cambia la posicion");
                } else {
                    System.out.println("Has introducido correctamente el portaaviones: ");
                }
            } else {
                System.err.println(
                        "No has introducido correctamente el portaaviones, tiene que estar dentro del tablero (10x10) y con una distancia de 3 ( 4 casillas ), en horizontal o vertical");
            }
    ...
    private static void Submarino(int tipo) {
        ...
        while (!colocar) { // pedir las posicion de inicio y final del submarino
            ...
            System.out.println("Introduce la columna");
            int columna_final = teclado.nextInt();
            if (Comprobar_Espacio_Barco(fila_inicial, columna_inicial, fila_final, columna_final, tipo)) { // El
                                                                                                           // submarino
                                                                                                           // encaja si
                                                                                                           // es true
                ...
                System.err.println(
                        "No has introducido correctamente el submarino, tiene que estar dentro del tablero (10x10) y con una distancia de 2 ( 3 casillas ), en horizontal o vertical");
            }
    ...
    private static void Lanchas() {
        ...
        while (fila < 0 || columna < 0 || fila > 9 || columna > 9) {
            ...
            if (fila < 0 || columna < 0 || fila > 9 || columna > 9) {
                System.out.println("Debe de meter una posición valida en la matriz entre 0 y 9");
            } else if (matriz[fila][columna] != 0) {
                System.out.println(
                        "No hay espacio disponible en las posiciones marcadas porque hay barcos ya, cambia la posicion");
            } else {
                System.out.println("Has introducido correctamente la lancha: ");
                matriz[fila][columna] = 8; // 8 es el codigo interno que tengo para decir que ahi hay una lancha
            }
        }
    ...
    /**
     * Funcion para meter internamente en el tablero los barcos segun una posicion
     * inicial y una final Devuelve con un boolean si hay espacio disponible para
     * colocar el barco o no
     *
     * @param fila_inicio
     ...
     * @return
     */
    private static boolean ColocarBarcos(int fila_inicio, int columna_inicio, int fila_final, int columna_final,
            int tipo, int matriz[][]) {
        // Confirmo de que fila_inicio < fila_final y columna_inicio < columna_final , y
        // si no las invierto para facilitar la logica y reducir el codigo
    ...
    private static void Tablero() {
        ...
        System.out.println("");
        System.out.print("\t");
        for (int i = 0; i < matriz.length; i++) { // Para mostrar los numeros de las columnas arriba del tablero para
                                                  // que sea más visual
            System.out.print(i + " ");
        }
        System.out.print("\t");
        for (int i = 0; i < matriz_cpu.length; i++) { // Para mostrar los numeros de las columnas arriba del tablero de
                                                      // la cpu para que sea más visual
            System.out.print(i + " ");
        }
        System.out.println("\n");
        for (int filas = 0; filas < matriz.length; filas++) {
            System.out.print(filas + "\t"); // Para mostrar los numeros de las filas a la izquierda para que sea más
                                            // visual
            for (int columnas = 0; columnas < matriz[filas].length; columnas++) {
                switch (matriz[filas][columnas]) {
                    ...
                    case 2 -> // El numero 2 para posiciones en las que hay un barco y ha sido hundido
                        System.out.print("H ");
                    case 3 -> // El numero 3 es para posiciones en las cuales la IA ha bombardeado de forma
                              // aleatoria
                        System.out.print("O ");
            ...
            System.out.print("\t" + filas); // Para mostrar los numeros de las filas a la derecha para que sea más
                                            // visual
     ...
     * @return
     */
    private static int Disparo(boolean esTurnoJugador, int[][] matriz_objetivo, int[][] portaaviones,
            int[][] submarino_1, int[][] submarino_2, int contador_barcos_hundidos) {

        boolean sitio_disparado = true;
        while (sitio_disparado) {
            if (esTurnoJugador) {
                Introducir_Fila_columna();
            } else {
            ...
            switch (matriz_objetivo[fila][columna]) {
                case 1 -> {
                    if (esTurnoJugador) {
                        System.out.println(
                                "En esta posicion ya has disparado y habia un barco que aun sigue en pie, pon otra posicion");
                ...
                case 2 -> {
                    if (esTurnoJugador) {
                        System.out.println(
                                "En esta posicion ya has disparado y habia un barco que ya ha sido hundido, pon otra posicion");
                        Introducir_Fila_columna();
                    }
                    sitio_disparado = true;
                ...
                case 3 -> {
                    if (esTurnoJugador) {
                        System.out.println(
                                "En esta posicion ya has disparado y no habia ningun barco, pon otra posicion");
                        Introducir_Fila_columna();
                    }
                    sitio_disparado = true;
                ...
                case 5 -> {
                    ...
                    switch (portaaviones[0][2]) { // En la posición [0][2] se almacena el numero de posiciones tocadas
                                                  // del barco
                        case 1 -> {
                        ...
                        case 4 -> {
                            ...
                            // Procedo a cambiar las posiciones guardadas que han sido tocadas a hundidas,
                            // porque se ha llegado a la longitud del barco
                ...
                case 6 -> {
                    ...
                    switch (submarino_1[0][2]) { // En la posición [0][2] se almacena el numero de posiciones tocadas
                                                 // del barco
                        case 1 -> {
                ...
                case 7 -> {
                    ...
                    switch (submarino_2[0][2]) { // En la posición [0][2] se almacena el numero de posiciones tocadas
                                                 // del barco
                        case 1 -> {
        ...
        return contador_barcos_hundidos; // Devuelvo el valor del contador ya que java la trata como variable local de
                                         // la funcion y no se me actualiza si no
    }
     ...
     * @return
     */
    private static boolean Comprobar_Espacio_Barco(int fila_inicial, int columna_inicial, int fila_final,
            int columna_final, int tipo) {
        int distancia = 0;
        if (tipo == 5) {
            distancia = 3; // Pongo 3 para el portaaviones porque es la distancia que tiene que haber de la
                           // casilla inicial a la fina, aunque en realidad tiene 4 casillas
        ...
        boolean filas_validas = fila_inicial >= 0 && fila_inicial < 10 && fila_final >= 0 && fila_final < 10; // Comprobamos
                                                                                                              // que
                                                                                                              // estan
                                                                                                              // dentro
                                                                                                              // de la
                                                                                                              ...
                                                                                                              // las
                                                                                                              // filas
        boolean columnas_validas = columna_inicial >= 0 && columna_inicial < 10 && columna_final >= 0
                && columna_final < 10; // Comprobamos que estan dentro de la matriz las columnas
        boolean distancia_4 = fila_final - fila_inicial == distancia || fila_inicial - fila_final == distancia
                || columna_final - columna_inicial == distancia || columna_inicial - columna_final == distancia; // si
                                                                                                                 // la
                                                                                                                 // distancia
                                                                                                                 // es
                                                                                                                 // exacta
                                                                                                                 // encaja
        boolean paralelo = fila_inicial == fila_final || columna_inicial == columna_final; // Comprueba si posiciona
                                                                                           // inicial y final forman una
                                                                                           // linea horizontal o
                                                                                           // vertical

        return filas_validas && columnas_validas && distancia_4 && paralelo; // Si el barco puede encajar sera true si
                                                                             // no false
    }
     ...
     */
    private static void Introducir_Fila_columna() {
        fila = -1;
